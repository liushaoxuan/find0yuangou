...
import com.ahxd.lingyuangou.ui.home.activity.ExchangeGiftListActivity;
 ...
 * GridView加载数据adapter
 */
public class ExchangeGiftCatAdapter extends BaseAdapter {

    private List<ExchangeBean.ExchangeCats> mData;
    private LayoutInflater mInflater;
    private Context mContext;
    private int mIndex;//页数下标，表示第几页，从0开始
    private int mPagerSize;//每页显示的最大数量

    public ExchangeGiftCatAdapter(Context context, List<ExchangeBean.ExchangeCats> listData, int mIndex, int mPagerSize) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        final ExchangeBean.ExchangeCats bean = mData.get(pos);
...
//                ToastUtils.showShort(mContext, pos + "");
                Intent intent = new Intent(mContext, ExchangeGiftListActivity.class);
                intent.putExtra("catName", bean.getCatName());
                intent.putExtra("goodsCatId", bean.getCatId());
